...
public class PreBuildCheck : IPreprocessBuildWithReport
{
    ...
    public void OnPreprocessBuild(BuildReport report)
    {
        ...
        if (PlayerSettings.applicationIdentifier != BachhhTools.GameSetting.packageName)
    ...
    {
        string[] requiredNetworks = await BachhhTools.GetRequiredNetworks();
        BachhhTools.GetMissingNetworks(requiredNetworks, missingNetworks =>
        {
    ...
    {
        BachhhTools.GameId ids;
        ids = await BachhhTools.GetText(BachhhTools.GameSetting.packageName);

        if (ids == null)
            throw new BuildFailedException("Can not get ads id from server!");

        if (BachhhTools.GameSetting.androidAppKey != ids.ironSourceAppKey)
        ...
        for (var i = 0; i < BachhhTools.GameSetting.aoaListIds.Count; i++)
        {
            if (BachhhTools.GameSetting.aoaListIds[i] != ids.aoaIds[i])
