...
#endif
{
    ...
    {
        BachhhDebug.Log("IAP >Initialized Success!", BachhhColor.Orange);
    ...
    {
        BachhhDebug.Log("IAP >OnInitializeFailed InitializationFailureReason:" + error);
    }
    ...
    public void BuyProduct(IapProductName id)
    {
        ...
        if (IsInitialized())
        {
            ...
            {
                BachhhDebug.Log(string.Format("IAP >Purchasing product asychronously: '{0}'", product.definition.id));
                _storeController.InitiatePurchase(product);
            }
            else
            {
                BachhhDebug.Log("IAP >BuyProductID: FAILED. Not purchasing product, either is not found or is not available for purchase");
            }
        }
        else
        {
            BachhhDebug.Log("IAP >BuyProductID FAILED. Not initialized.");
        }
    }
    ...
    public void RestorePurchases()
    {
        if (!IsInitialized())
        {
            BachhhDebug.Log("IAP >RestorePurchases FAIL. Not initialized.");
            return;
        }
        ...
        {
            BachhhDebug.Log("IAP >RestorePurchases started ...");

            var apple = _storeExtensionProvider.GetExtension<IAppleExtensions>();

            apple.RestoreTransactions(result => { BachhhDebug.Log("IAP >RestorePurchases continuing: " + result + ". If no further messages, no purchases available to restore."); });
        }
        else
        {
            BachhhDebug.Log("IAP >RestorePurchases FAIL. Not supported on this platform. Current = " + Application.platform);
        }
    }
    ...
    public PurchaseProcessingResult ProcessPurchase(PurchaseEventArgs args)
    {
        try
        {
            ...
            BachhhDebug.Log("IAP >Receipt: " + googleReceipt.TransactionID + " ", BachhhColor.Yellow);
        ...
        {
            BachhhDebug.Log("IAP >Failed " + e.Message, BachhhColor.Red);
    ...
    {
        BachhhDebug.Log(string.Format("IAP >OnPurchaseFailed: FAIL. Product: '{0}', PurchaseFailureReason: {1}", product.definition.storeSpecificId,
            failureReason), BachhhColor.Red);
    }
